...
use crate::keyboard::uart_serial::{UartSerial, UartSerialRequest, UartSerialResponse};
use crate::keyboard::usb_serial::{UsbSerial, UsbSerialDevice};

#[inline(never)]
pub fn run_left(mut usb_serial: UsbSerial, mut usb_dev: UsbSerialDevice, _oled_handle: OledHandle, mut uart_driver: UartSerial, mut left_buttons: LeftButtons, mut power_led_pin: PowerLed, timer: Timer) {
    ...
    loop {
        ...
        if output_all {
            ...
            if !has_sent_ping {
                uart_driver.send_msg(UartSerialRequest::Ping);
                let _ = usb_serial.write_str("Sent ping\r\n");
                has_sent_ping = true;
            } else {
                if let Some(UartSerialResponse::Pong) = uart_driver.recv() {
